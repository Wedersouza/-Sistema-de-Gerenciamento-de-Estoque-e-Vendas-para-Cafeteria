...
router.post("/fornecedores", requireAdmin, (req, res) => {
...
router.patch("/fornecedores/:id/inativar", requireAdmin, (req, res) => {
...
router.patch("/fornecedores/:id/reativar", requireAdmin, (req, res) => {
  db.prepare("UPDATE fornecedores SET ativo = 1 WHERE id = ?").run(req.params.id);
  res.json({ ok: true });
});
